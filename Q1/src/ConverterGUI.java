...
//                outputField.setText(text + "Happy Halloween!");
                System.out.println(outputUnit.getSelectedItem());
                ...
                try {
                    double km = Double.parseDouble(inputField.getText()); // Parse the input from the text field as a double
                    if (km < 0) {
                        outputField.setText("Please enter a positive number."); // Validation for negative numbers
                        return;
                    }

                    String selectedUnit =  (String)outputUnit.getSelectedItem(); // Get the selected unit from the dropdown
                    Client.convert(inputField, selectedUnit, outputField); // Start the processing chain with the mile handler
                } catch (NumberFormatException e) {
                    outputField.setText("Invalid input. Please enter a valid number."); // Catch and handle invalid number format
                }
